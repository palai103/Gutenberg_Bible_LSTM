...
def crop_images(input_path, output_path):
    for file_name in os.listdir(input_path):
        if file_name.endswith('.jpg') or file_name.endswith('.png'):
            ...
            crop_image = image[mask_odd_page] if (int(file_name[-5]) % 2 == 1) else image[mask_even_page]
            cv2.imwrite(os.path.join(output_path, str(file_name[:-4]) + '.png'), crop_image)
...
def deskew_images(input_path, output_path):
    for file_name in sorted(os.listdir(input_path)):
        if file_name.endswith('.jpg') or file_name.endswith('.png'):
            ...
            print('Best angle:' + str(best_angle) + ' ' + str(file_name))
            ...
            image.save(os.path.join(os.path.abspath(output_path), str(output_file) + '.png'))
...
def prepare_images(datasets):
    for dataset in datasets:
        input_path = pathlib.Path("../dataset/original/" + dataset)
        output_path = pathlib.Path("../dataset/deskewed/" + dataset)
